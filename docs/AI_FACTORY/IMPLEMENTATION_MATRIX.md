...
| Fixed-map batch generation pipeline | implemented | `audio_engine/integration/asset_pipeline.py`, `audio_engine/integration/game_state_map.py` | 2026-05-14 | `pytest` | Works for current integration map; request-batch path now also implemented. |
| Request-batch generation (factory-input driven) | implemented | `audio_engine/integration/asset_pipeline.py` (`execute_request_batch`), `audio_engine/cli.py` (`generate-request-batch`), `tests/test_integration.py`, `tests/test_engine_cli.py` | 2026-05-14 | `pytest` + `/tmp` smoke run | Per-request seeds explicit; `RequestBatchRecord`/`RequestBatchResult` for machine-readable results. OGG requires optional `soundfile` dep. |
...
| Generation request format with seed/provenance fields | partial | `docs/AI_FACTORY/SCHEMAS/GENERATION_REQUEST_SCHEMA.md`, `audio_engine/integration/factory_inputs.py`, `audio_engine/integration/asset_pipeline.py` | 2026-05-14 | `pytest` | Typed batch/request loading and deterministic execution exist for the committed music and SFX fixtures; provenance log writing remains the next session. |
...
| Capability | State | Source of truth | Last verified | Verification command | Notes / next step |
|---|---|---|---|---|---|
| Plan-driven batch orchestration | partial | `docs/AI_FACTORY/ROADMAP.md`, `audio_engine/integration/asset_pipeline.py` | 2026-05-14 | `pytest` | Request-batch execution path implemented; full plan-driven orchestration (joining plan duration targets to requests) is still future work. |
