...
| Machine-readable review-log writer | implemented | `audio_engine/integration/asset_pipeline.py` (`ReviewLogWriter`), `audio_engine/cli.py` (`write-review-log`) | 2026-05-15 | `pytest`, CLI smoke run | Writes/updates stable review-log JSON from provenance sidecars, optional `qa-batch` report snapshots, and optional variant-family decisions. |
| Review-log handoff integration in approval/export | implemented | `audio_engine/integration/asset_pipeline.py` (`ApprovalWorkflow.approve_batch`, `DraftExportPipeline.export`), `audio_engine/cli.py` (`approve-draft`, `export-drafts`) | 2026-05-15 | `pytest` | Additive optional flags update review logs during approval/export without breaking existing behavior. |
...
| Backend evaluation + dependency/availability guidance | implemented | `audio_engine/ai/backend.py`, `audio_engine/cli.py`, `docs/AI_FACTORY/SUBSYSTEMS/MUSIC.md` | 2026-05-15 | `pytest` | Backend registry now exposes executable evaluation metadata (`available`, supported modalities, dependency summary, availability reason) and `list-backends` surfaces it in CLI output. |
| Repeated-SFX variation strategy guidance | implemented (factory-side) | `audio_engine/integration/factory_inputs.py`, `audio_engine/integration/asset_pipeline.py`, `docs/AI_FACTORY/SUBSYSTEMS/SFX.md` | 2026-05-15 | `pytest` | Request parsing now enforces repeated-SFX variant-family rules (`_varNN`, distinct seeds, contiguous indices) and provenance now records `variationFamily`/`variationIndex`; runtime in-game variant selection remains downstream. |
| Category-specific SFX loudness/readability targets | implemented (docs) | `docs/AI_FACTORY/SUBSYSTEMS/SFX.md`, `docs/AI_FACTORY/QA/QUALITY_BARS.md` | 2026-05-15 | Manual doc review | Category guidance now maps acceptance profiles to review loudness/readability targets while preserving existing global QA gates. |
| AI factory review-state workflow | implemented | `audio_engine/integration/asset_pipeline.py` (`ReviewLogWriter`), `audio_engine/cli.py` (`write-review-log`, review-log flags), `docs/AI_FACTORY/QA/REVIEW_WORKFLOW.md` | 2026-05-15 | `pytest`, CLI smoke run | Review/report template guidance now has executable writing/update paths aligned with provenance and optional qa-batch snapshots. |
