...
| `SESSION-002` | 2026-05-14 | completed | Added `execute_request_batch()` API + `generate-request-batch` CLI command. Per-request seeds explicit. `RequestBatchRecord`/`RequestBatchResult` for machine-readable results. 11 new tests. 332 passed. SFX 5/5 OK smoke run. | this PR |
